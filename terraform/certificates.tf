...
data "template_file" "certificates" {
  ...
  vars = {
    ...
    etcd1_ip       = length(aws_instance.etcd) > 1 ? aws_instance.etcd[1].private_ip : ""
    etcd2_ip       = length(aws_instance.etcd) > 2 ? aws_instance.etcd[2].private_ip : ""
    controller0_ip = aws_instance.controller[0].private_ip
    controller1_ip = length(aws_instance.controller) > 1 ? aws_instance.controller[1].private_ip : ""
    controller2_ip = length(aws_instance.controller) > 2 ? aws_instance.controller[2].private_ip : ""
    worker0_ip     = aws_instance.worker[0].private_ip
    worker1_ip     = length(aws_instance.worker) > 1 ? aws_instance.worker[1].private_ip : ""
    worker2_ip     = length(aws_instance.worker) > 2 ? aws_instance.worker[2].private_ip : ""

    etcd0_dns       = aws_instance.etcd[0].private_dns
    etcd1_dns       = length(aws_instance.etcd) > 1 ? aws_instance.etcd[1].private_dns : ""
    etcd2_dns       = length(aws_instance.etcd) > 2 ? aws_instance.etcd[2].private_dns : ""
    controller0_dns = aws_instance.controller[0].private_dns
    controller1_dns = length(aws_instance.controller) > 1 ? aws_instance.controller[1].private_dns : ""
    controller2_dns = length(aws_instance.controller) > 2 ? aws_instance.controller[2].private_dns : ""
    worker0_dns     = aws_instance.worker[0].private_dns
    worker1_dns     = length(aws_instance.worker) > 1 ? aws_instance.worker[1].private_dns : ""
    worker2_dns     = length(aws_instance.worker) > 2 ? aws_instance.worker[2].private_dns : ""
  }
}
