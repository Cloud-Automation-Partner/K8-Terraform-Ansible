...
data "template_file" "ssh_cfg" {
  ...
  vars = {
    ...
    etcd1_ip       = length(aws_instance.etcd) > 1 ? aws_instance.etcd[1].public_ip : ""
    etcd2_ip       = length(aws_instance.etcd) > 2 ? aws_instance.etcd[2].public_ip : ""
    controller0_ip = aws_instance.controller[0].public_ip
    controller1_ip = length(aws_instance.controller) > 1 ? aws_instance.controller[1].public_ip : ""
    controller2_ip = length(aws_instance.controller) > 2 ? aws_instance.controller[2].public_ip : ""
    worker0_ip     = aws_instance.worker[0].public_ip
    worker1_ip     = length(aws_instance.worker) > 1 ? aws_instance.worker[1].public_ip : ""
    worker2_ip     = length(aws_instance.worker) > 2 ? aws_instance.worker[2].public_ip : ""
  }
}
